...
<Card class="space-y-3 {className}">
  ...
  <div class="flex flex-wrap items-start justify-between gap-4">
    <div class="space-y-1.5 max-w-2xl">
      <div class="flex items-center gap-2">
        <div class="flex h-7 w-7 items-center justify-center rounded bg-sky-500/10 text-sky-600 dark:text-sky-400 border border-sky-500/20">
          <Shield class="h-4 w-4" />
        </div>
        <h2 class="text-lg font-bold text-zinc-900 dark:text-zinc-100 tracking-tight">
  ...
  <div class="flex flex-wrap items-center gap-2 pt-2 border-t border-zinc-200 dark:border-zinc-800/80 text-xs">
    <!-- Destination pills -->
    <Badge variant="outline" size="sm" class="gap-1 font-mono text-[11px] text-zinc-600 dark:text-zinc-400">
      <Folder class="h-3 w-3 text-zinc-400 dark:text-zinc-500" />
      {folderStatusLabel}
    </Badge>

    <Badge variant="outline" size="sm" class="gap-1 font-mono text-[11px] text-zinc-600 dark:text-zinc-400">
      <Globe class="h-3 w-3 text-zinc-400 dark:text-zinc-500" />
      {httpsStatusLabel}
    </Badge>
    ...
    {#each frontmatterEntries as [key, value]}
      {#if key !== 'description'}
        <span class="inline-flex items-center rounded bg-zinc-100 dark:bg-zinc-800/60 border border-zinc-200 dark:border-zinc-700/50 px-2 py-0.5 text-[11px] font-mono text-zinc-700 dark:text-zinc-300">

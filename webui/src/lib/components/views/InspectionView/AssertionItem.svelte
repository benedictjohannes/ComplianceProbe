...
<div
  class={cn(
    'rounded-lg border border-zinc-800 bg-zinc-900/60 p-3 transition-colors hover:border-zinc-700',
    className
  )}
>
  ...
  <div class="flex items-start justify-between gap-3">
    ...
    >
      <span class="mt-0.5 text-zinc-500 group-hover:text-zinc-300 transition-colors">
        {#if isExpanded}
      ...
      <div class="space-y-1 flex-1">
        <div class="flex flex-wrap items-center gap-2">
          <span class="font-mono text-xs text-sky-400 bg-sky-500/10 border border-sky-500/20 px-2 py-0.5 rounded font-semibold select-all">
            {assertion.code}
          </span>
    ...
    <div class="flex items-center gap-2 shrink-0">
      ...
      <span class="rounded-full bg-zinc-800 border border-zinc-700/60 px-2 py-0.5 text-xs font-mono text-zinc-400">
  ...
  {#if isExpanded}
    <div class="mt-3 pt-3 border-t border-zinc-800/80 space-y-3 pl-6 text-sm text-zinc-300">
      {#if assertion.description}
        <p class="text-zinc-300 text-xs leading-relaxed">
          <strong class="text-zinc-400 font-semibold">Description:</strong> {assertion.description}
        </p>
      {/if}
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-2 text-xs">
        {#if assertion.passDescription}
          <div class="rounded bg-emerald-950/40 border border-emerald-500/20 p-2 text-emerald-300">
            <span class="font-semibold text-emerald-400 block mb-0.5">✓ Pass Criteria:</span>
            {assertion.passDescription}
          </div>
        {/if}

        {#if assertion.failDescription}
          <div class="rounded bg-rose-950/40 border border-rose-500/20 p-2 text-rose-300">
            <span class="font-semibold text-rose-400 block mb-0.5">✕ Fail Criteria:</span>
      ...
      {#if assertion.minPassingScore !== undefined}
        <div class="text-xs font-mono text-zinc-400">
          Min Passing Score: <span class="text-zinc-200 font-semibold">≥ {assertion.minPassingScore} points</span>
        </div>
      {/if}
        ...
        <div class="space-y-3 pt-2">
          ...
          {#if assertion.preCmds && assertion.preCmds.length > 0}
            {#each assertion.preCmds as exec, i (i)}
              <div class="rounded-md border border-zinc-800 bg-zinc-950/70 p-2.5 space-y-2 text-xs font-mono">
                <div class="flex flex-wrap items-center justify-between gap-2 border-b border-zinc-800/80 pb-1.5 text-zinc-400">
                  <span class="font-semibold text-sky-400">
                    [Pre-Command {i + 1}/{assertion.preCmds.length}]
                  </span>
                  <div class="flex items-center gap-1.5 text-[11px]">
                    {#if exec.shell}
                      <span class="bg-zinc-800 px-1.5 py-0.5 rounded text-zinc-300">Shell: {exec.shell}</span>
                    {/if}
                ...
                {#if exec.script}
                  <div class="rounded bg-zinc-900 border border-zinc-800/80 p-2 text-zinc-200 overflow-x-auto select-text whitespace-pre-wrap">
                    {exec.script}
                  </div>
                {/if}

                {#if exec.gather && exec.gather.length > 0}
                  <div class="space-y-1 text-zinc-400 text-[11px] pt-1">
                    {#each exec.gather as g}
                      <div>
                        Gather: var <span class="text-sky-300">"{g.key}"</span>
                        {#if g.regex}
                          &lt;= regex <code class="text-amber-300">/{g.regex}/</code>
                        {/if}
                ...
                <div class="flex flex-wrap items-center justify-between gap-2 border-b border-zinc-800/80 pb-1.5 text-zinc-400">
                  ...
                  <div class="flex items-center gap-1.5 text-[11px]">
                    {#if cmd.passScore !== undefined || cmd.failScore !== undefined}
                      <span class="bg-zinc-800 px-1.5 py-0.5 rounded text-zinc-300">
                        Score: +{cmd.passScore ?? 1} / -{cmd.failScore ?? 1}
                      </span>
                    {/if}
                    {#if cmd.exec?.shell}
                      <span class="bg-zinc-800 px-1.5 py-0.5 rounded text-zinc-300">Shell: {cmd.exec.shell}</span>
                    {/if}
                  ...
                  {#if cmd.stdOutRule}
                    <div class="flex items-center gap-1">
                      ...
                      {#if cmd.stdOutRule.regex}
                        <span class="text-amber-300">regex /{cmd.stdOutRule.regex}/</span>
                      {:else if cmd.stdOutRule.func}
                        <span class="text-sky-300">func()</span>
                      {/if}
                    </div>
                  {/if}
                  ...
                  {#if cmd.stdErrRule}
                    <div class="flex items-center gap-1">
                      ...
                      {#if cmd.stdErrRule.regex}
                        <span class="text-rose-300">regex /{cmd.stdErrRule.regex}/</span>
                      {/if}
                    </div>
                  {/if}

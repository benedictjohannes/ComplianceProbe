...
// Options configures the runner behavior for probe or builder.
type Options struct {
	Name           string
	IsAgent        bool
	PreprocessHook func(config *playbook.Playbook, baseDir string) error
	CustomFlags    func(fs *flag.FlagSet)
	CustomHandler  func(fs *flag.FlagSet) (handled bool, exitCode int)
}

// Run parses arguments and executes either worker, custom action, UI server, or headless run.
func Run(args []string, opts Options) int {
	...
	if opts.CustomFlags != nil {
		opts.CustomFlags(flags)
	}
	...
	if opts.CustomHandler != nil {
		if handled, code := opts.CustomHandler(flags); handled {
			return code
		}
	}
	...
	if !isUI && configPath == "" && len(args) == 0 {
		if desktop.IsDesktopGUI() {
			...
			fmt.Printf("❌ Error: No playbook provided. Use '%s [path/to/playbook.yaml]' or run '%s --ui'\n", name, name)
			return 1
		}
	...
	// Optional CLI in-memory preprocess hook for local authoring
	if opts.PreprocessHook != nil {
		if err := opts.PreprocessHook(config, filepath.Dir(configPath)); err != nil {

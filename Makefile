...
build-webui: ## Build webui frontend assets into internal/webui/dist
	cd webui && npm run build

## build: Build production binaries for all platforms
build: build-webui build-linux build-windows build-mac-intel build-mac-arm ## Build production binaries for all platforms

build-linux: build-webui ## Build production for Linux (amd64)
	GOOS=linux GOARCH=amd64 go build $(BUILD_FLAGS) -o $(BINARY_NAME)-linux ./cmd/probe

build-windows: build-webui ## Build production for Windows (amd64)
	GOOS=windows GOARCH=amd64 go build $(BUILD_FLAGS) -o $(BINARY_NAME)-windows.exe ./cmd/probe

build-mac-intel: build-webui ## Build production for Mac Intel (amd64)
	GOOS=darwin GOARCH=amd64 go build $(BUILD_FLAGS) -o $(BINARY_NAME)-mac-intel ./cmd/probe

build-mac-arm: build-webui ## Build production for Mac Arm (arm64)
...
build-builder: build-webui build-builder-linux build-builder-windows build-builder-mac-intel build-builder-mac-arm ## Build builder binaries for all platforms

build-builder-linux: build-webui ## Build builder for Linux (amd64)
	GOOS=linux GOARCH=amd64 go build $(BUILD_FLAGS) -o $(BINARY_NAME)-builder-linux ./cmd/builder

build-builder-windows: build-webui ## Build builder for Windows (amd64)
	GOOS=windows GOARCH=amd64 go build $(BUILD_FLAGS) -o $(BINARY_NAME)-builder-windows.exe ./cmd/builder

build-builder-mac-intel: build-webui ## Build builder for Mac Intel (amd64)
	GOOS=darwin GOARCH=amd64 go build $(BUILD_FLAGS) -o $(BINARY_NAME)-builder-mac-intel ./cmd/builder

build-builder-mac-arm: build-webui ## Build builder for Mac Arm (arm64)
